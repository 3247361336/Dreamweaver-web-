...
class StudentController extends Controller {
    ...
    public function showList() {
       ...
       // $model = M('book');
        ...
        //$book_id = I('param.book_id', 2);
        //以数组的形式组合查询条件
       // $where = array('book_id' => $book_id);
        //通过模型类获取指定班级ID的学生信息
      //  $book_info = $model->where($where)->select();
        $book_info = D('book')->select();
        //把班级ID分配到视图页面
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($model->add()) {
                ...
                $this->success('学生添加成功，正在跳转，请稍候！', U("showList?book_id={$book_id}"));
                return;
            }
            //添加失败，则返回到上一页面
    ...
    public function update() {
        ...
        if (IS_POST) {
            //使用create方法获取表单数据
            $book_info = $model->create();
            //使用save方法进行数据更新
            if ($model->save() !== false) {
                //更新成功，则提示相关信息并跳转到当前学生所属班级的学生列表页
                $this->success('学生信息更新成功，正在跳转，请稍候！', U("showList?book_id={$book_info['book_id']}"));
                return;
            }
            //更新失败，提示相关信息并跳转到上一页面
        ...
        $book_info = $model->where($where)->find();
	...
	public function delete() {
		...
		$this->success('删除成功，正在跳转，请稍候！', U("showList?book_id={$book_id}"));
		return;
	}
	...
	public function addAll(){
		...
		//实例化Major模型对象，使用relation方法进行关联操作
		$major_info = D('book')->relation(true)->select();
		//将专业班级信息分配到视图页面中
		$this->assign('book_info', $book_info);
		//显示视图文件
		$this->display();
